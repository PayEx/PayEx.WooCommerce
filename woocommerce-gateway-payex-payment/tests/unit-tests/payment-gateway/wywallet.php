...
class WC_Tests_Payment_WyWallet extends WC_Payment_Unit_Test_Case {
	...
	public function test_wc_payment_payex_order() {
		...
		// Set payment gateway
		$order->set_payment_method( $payment_gateways['payex_wywallet'] );

		$this->assertInstanceOf( 'WC_Order', wc_get_order( $order->id ) );
		$this->assertEquals( 'payex_wywallet', $order->payment_method );
	}
	...
	public function test_wc_payment_payex_checkout() {
		...
		$_POST['payment_method'] = 'payex_wywallet';
